...
const iyzicoCallbackSecret = process.env.IYZICO_CALLBACK_SECRET || iyzicoSecretKey;
...
// TEMP DEV ONLY — disable before production. When enabled, /api/auth/dev-bypass
// mints a session for the seeded owner without a password. Keep this off (unset
// or "0"/"false") in any production environment.
const bypassLogin = ["1", "true", "yes"].includes(String(process.env.HOMETASTE_BYPASS_LOGIN || "").toLowerCase());
const bypassRole = (process.env.HOMETASTE_BYPASS_ROLE || "customer").trim().toLowerCase();
const bypassUserEmail = (process.env.HOMETASTE_BYPASS_USER_EMAIL || "").trim().toLowerCase();
...
// A stored hash is `${salt}:${hash}` where hash is an even-length hex string.
const isUsablePasswordHash = (stored) => {
  if (typeof stored !== "string") return false;
  const [salt, hash] = stored.split(":");
  return Boolean(salt) && typeof hash === "string" && hash.length > 0 && hash.length % 2 === 0 && /^[0-9a-f]+$/i.test(hash);
};

// Never throws: missing, malformed, or wrong-length hashes all return false so a
// bad stored hash produces a clean 401 instead of a 500 during login.
...
const maxImageDataUrlBytes = 2 * 1024 * 1024;
const allowedImageMimeTypes = new Set(["image/jpeg", "image/png", "image/webp", "image/gif"]);
const roundMoney = (value) => Math.round((Number(value || 0) + Number.EPSILON) * 100) / 100;
const gatewayProviders = new Set(["stripe", "iyzico", "paytr"]);
const manualPaymentProviders = new Set(["cash_on_delivery", "bank_transfer", "manual_card"]);
function orderAmounts(subtotal) {
  const safeSubtotal = roundMoney(subtotal);
  const appCommissionAmount = roundMoney(safeSubtotal * commissionRate);
  const deliveryFee = safeSubtotal > 0 ? deliveryFeeAmount : 0;
  return {
    subtotal: safeSubtotal,
    appCommissionRate: commissionRate,
    appCommissionAmount,
    deliveryFee,
...
function validateImageValue(value, label = "Image") {
  ...
  if (/^data:/i.test(text)) {
    ...
    if (approxBytes > maxImageDataUrlBytes) throw new Error(`${label} must be 2 MB or smaller.`);
    return text;
  }
...
const paymentMethods = ["cash", "card", "iban", "stripe", "iyzico", "paytr", "visa", "mastercard", "troy", "apple_pay", "google_pay", "turkish_bank_card"];
...
const timelineEvents = new Set(["order_created", "payment_pending", "payment_confirmed", "payment_failed", "payment_refunded", "order_accepted", "order_delivered", "payout_released", "payout_refunded", "provider_callback_received", "provider_callback_rejected", "manual_admin_action"]);
...
function isPublicDish(dish) {
  return dish?.status === "approved" && dish.available !== false;
}

function isApprovedDish(dish) {
  return dish?.status === "approved";
}

function dishCookSnapshot(db, cook) {
...
function publicDishForUser(dish, user, ownCook = null) {
...
function orderRequestSnapshot(db, user, dish, cook, input, createdAt) {
  return {
    orderId: "",
    customerId: user.id,
    ...
    cookName: cook?.name || "",
    cookPhone: cook?.phone || db.users.find((item) => item.id === cook?.userId)?.phone || "",
    deliveryAddress: String(input.deliveryAddress || "").trim(),
    deliveryLocation: normalizeLocation(input.customerLocation, String(input.deliveryAddress || "")),
...
function cleanupDemoDataInMemory(db) {
  ...
  const demoTimelineIds = new Set((db.paymentTimeline || []).filter((event) => demoOrderIds.has(event.orderId) || demoPaymentIds.has(event.paymentId)).map((event) => event.id));
  const demoAuditIds = new Set((db.auditLogs || []).filter((entry) => demoOrderIds.has(entry.orderId) || demoPaymentIds.has(entry.paymentId) || demoUserIds.has(entry.actorId)).map((entry) => entry.id));
  ...
  db.paymentTimeline = (db.paymentTimeline || []).filter((item) => !demoTimelineIds.has(item.id));
  db.auditLogs = (db.auditLogs || []).filter((item) => !demoAuditIds.has(item.id));
  ...
  return {
    ...
    paymentTimeline: demoTimelineIds,
    auditLogs: demoAuditIds,
...
function collectCascadeForCooks(db, cookIdsInput, bucket = null) {
  ...
  const timelineIds = bucket?.paymentTimeline || new Set();
  const auditIds = bucket?.auditLogs || new Set();
  ...
  (db.paymentTimeline || []).filter((event) => orderIds.has(event.orderId) || paymentIds.has(event.paymentId)).forEach((event) => timelineIds.add(event.id));
  (db.auditLogs || []).filter((entry) => orderIds.has(entry.orderId) || paymentIds.has(entry.paymentId)).forEach((entry) => auditIds.add(entry.id));
  ...
  return { users: bucket?.users || new Set(), cooks: cookIds, dishes: dishIds, orders: orderIds, messages: messageIds, notifications: notificationIds, sessions: bucket?.sessions || new Set(), mealPlans: mealPlanIds, subscriptions: subscriptionIds, payments: paymentIds, refunds: refundIds, paymentTimeline: timelineIds, auditLogs: auditIds, socialActions: socialIds, authTokens: bucket?.authTokens || new Set(), notificationDevices: bucket?.notificationDevices || new Set() };
}
...
function collectCascadeForUsers(db, userIdsInput) {
  ...
  (db.paymentTimeline || []).filter((event) => removed.orders.has(event.orderId) || removed.payments.has(event.paymentId)).forEach((event) => removed.paymentTimeline.add(event.id));
  (db.auditLogs || []).filter((entry) => removed.orders.has(entry.orderId) || removed.payments.has(entry.paymentId) || removed.users.has(entry.actorId)).forEach((entry) => removed.auditLogs.add(entry.id));
...
function applyCascadeRemoval(db, removed) {
  ...
  db.paymentTimeline = (db.paymentTimeline || []).filter((item) => !removed.paymentTimeline.has(item.id));
  db.auditLogs = (db.auditLogs || []).filter((item) => !removed.auditLogs.has(item.id));
...
function isGatewayProvider(provider) {
  return gatewayProviders.has(provider);
}

function initialPaymentStatus(provider) {
  if (provider === "cash_on_delivery") return "cash_on_delivery";
  return "pending";
}
...
function syncOrderPayment(order, payment) {
  ...
  order.payment = {
    ...
    provider: payment.provider || order.payment?.provider || "manual",
    method: payment.method || order.paymentMethod || "cash",
    externalPaymentId: payment.externalPaymentId || "",
    providerReference: payment.providerReference || payment.externalPaymentId || "",
    checkoutUrl: payment.checkoutUrl || "",
    metadata: payment.metadata || order.payment?.metadata || {},
    gross: payment.gross,
    foodAmount: payment.foodAmount,
    deliveryFee: payment.deliveryFee,
    appCommissionRate: payment.appCommissionRate,
    appCommissionAmount: payment.appCommissionAmount,
    totalAmount: payment.totalAmount,
    commissionRate: payment.commissionRate,
    commission: payment.commission,
    cookPayout: payment.cookPayout,
    capturedAt: payment.capturedAt || order.payment?.capturedAt || null,
    releasedAt: payment.releasedAt || null,
    refundStatus: payment.refundStatus || order.payment?.refundStatus || "none"
  };
  order.paymentStatus = order.payment.paymentStatus;
  order.payoutStatus = order.payment.payoutStatus;
}

function maybeReleasePayout(order, payment, at = now(), db = null) {
  ...
  payment.releasedAt = at;
  order.payoutStatus = "released";
  syncOrderPayment(order, payment);
  if (db) addPaymentTimeline(db, { orderId: order.id, paymentId: payment.id, type: "payout_released", provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: "Payout released after confirmed payment and delivery." });
  return true;
}

function setPaymentStatus(db, payment, status, details = {}) {
  const order = db.orders.find((item) => item.id === payment.orderId);
  const at = details.at || now();
  const oldPaymentStatus = payment.paymentStatus || payment.status || "pending";
  const oldPayoutStatus = payment.payoutStatus || "pending";
  payment.paymentStatus = status;
  payment.status = status;
  payment.updatedAt = at;
  if (details.externalPaymentId) payment.externalPaymentId = details.externalPaymentId;
  if (details.providerReference) payment.providerReference = details.providerReference;
  payment.metadata = { ...(payment.metadata || {}), ...(details.metadata || {}) };
  if (status === "paid") payment.capturedAt = at;
  if (status === "failed") payment.payoutStatus = "pending";
  if (status === "refunded") payment.payoutStatus = "refunded";
  if (order) {
    if (status === "refunded") order.payoutStatus = "refunded";
    maybeReleasePayout(order, payment, at, db);
    syncOrderPayment(order, payment);
  }
  const type = status === "paid" ? "payment_confirmed" : status === "failed" ? "payment_failed" : status === "refunded" ? "payment_refunded" : "payment_pending";
  addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type, actorId: details.actorId || "", provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: details.note || details.metadata?.adminNote || details.metadata?.manualConfirmationNote || "" });
  if (details.audit !== false) addAuditLog(db, {
    ...
    oldPayoutStatus,
    newPayoutStatus: payment.payoutStatus || "pending",
    provider: payment.provider,
    providerReference: payment.providerReference || payment.externalPaymentId || "",
    note: details.note || "",
    requestMeta: details.requestMeta || {}
  });
  return order;
}
...
function addPaymentTimeline(db, event) {
  db.paymentTimeline ||= [];
  const type = timelineEvents.has(event.type) ? event.type : "manual_admin_action";
  db.paymentTimeline.unshift({
    id: event.id || id("tle"),
...
function addAuditLog(db, entry) {
  ...
  db.auditLogs.unshift({
    ...
    actionType: entry.actionType || "manual_admin_action",
    oldPaymentStatus: entry.oldPaymentStatus || "",
    newPaymentStatus: entry.newPaymentStatus || "",
    oldPayoutStatus: entry.oldPayoutStatus || "",
    newPayoutStatus: entry.newPayoutStatus || "",
...
function submittedCoordinates(value) {
  if (value && typeof value === "object") {
...
// In Supabase/production mode the login shortcut (fastSupabaseLogin) runs before
// loadDb()/ensureSystemUsers(), so seeded owner/cook/driver accounts could be
// missing on the very first login. This ensures they exist in app_users first.
// It runs at most once per process (cached promise) and never logs credentials.
// Local JSON mode does not need this: there, ensureSystemUsers() already runs on
// every request before the login handler.
let seedUsersReadyPromise = null;
async function ensureSeedUsersReady() {
  if (!useSupabase || !seedAccounts.length) return;
  if (!seedUsersReadyPromise) {
    seedUsersReadyPromise = (async () => {
      const emails = seedAccounts.map((account) => account.email.trim().toLowerCase());
      const inList = emails.map((email) => `"${email.replace(/"/g, '\\"')}"`).join(",");
      const existing = await supabaseRequest("app_users", {
        query: `?email=in.(${encodeURIComponent(inList)})&select=*`
      }).catch(() => []);
      const byEmail = new Map((existing || []).map((row) => [String(row.email || "").toLowerCase(), row]));
      const pending = [];
      for (const account of seedAccounts) {
        const normalizedEmail = account.email.trim().toLowerCase();
        const row = byEmail.get(normalizedEmail);
        if (!row) {
          pending.push({
            id: account.id,
            name: account.name,
        ...
        // Existing seed account: repair only a missing/malformed/mismatched hash
        // so the seeded password keeps working, leaving other fields untouched.
        if (!verifyPassword(account.password, row.password_hash)) {
          pending.push({ ...toUser(row), passwordHash: hashPassword(account.password) });
        }
      }
      if (pending.length) {
        // Mirror saveSupabaseDb's tolerance for older schemas that lack the
        // newer app_users columns (auth_meta, profile_photo, ...).
        try {
          await upsert("app_users", pending.map(fromUser));
        } catch (err) {
          console.warn(`Falling back to legacy app_users payload: ${err.message}`);
          await upsert("app_users", pending.map(fromUserLegacy));
        }
      }
    })().catch((err) => {
      // Reset so a transient Supabase error can be retried on the next login.
      seedUsersReadyPromise = null;
      console.warn(`[auth] could not ensure seed users: ${err.message}`);
      throw err;
    });
  }
  return seedUsersReadyPromise;
}

function paymentLedgerForOrder(order) {
  const amounts = orderAmounts(order.subtotal || 0);
  const foodAmount = amounts.subtotal;
  const provider = paymentProviderFor(order.paymentMethod || "cash");
  const normalizedProvider = provider === "cash" ? "cash_on_delivery" : provider || "manual";
  const status = initialPaymentStatus(normalizedProvider);
  return {
    ...
    cookPayout: roundMoney(Math.max(0, foodAmount - amounts.appCommissionAmount)),
    provider: normalizedProvider,
    capturedAt: status === "cash_on_delivery" ? order.createdAt || now() : null,
    releasedAt: null,
    refundStatus: "none"
  };
}
...
function normalizeDb(db) {
  ...
  db.socialActions = db.socialActions.filter((action) => {
    if (action.type === "favorite_cook" || action.type === "favorite_dish") {
      ...
      if (!action.userId || !target || seenFavorites.has(key)) return false;
      seenFavorites.add(key);
      return true;
    }
    if (action.type !== "follow") return true;
    const key = `${action.userId}:${action.cookId}`;
    if (!action.userId || !action.cookId || seenFollows.has(key)) return false;
    seenFollows.add(key);
    return true;
  });
  db.authTokens ||= [];
  ...
  for (const cook of db.cooks) {
    ...
    cook.followers ||= 0;
    cook.email ||= cookUser?.email || "";
    cook.phone ||= cookUser?.phone || "";
    cook.country ||= cookUser?.country || "TR";
    cook.district ||= "";
    cook.fullAddress ||= "";
    cook.location ||= null;
    cook.profilePhoto ||= cookUser?.profilePhoto || "";
    cook.coverPhoto ||= cookUser?.profileCover || "";
    cook.requestDate ||= cook.createdAt || now();
    cook.requestStatus ||= cook.status || "pending";
    cook.online = Boolean(cook.online);
    cook.name ||= cookUser?.name || "HomeTaste cook";
  }
  for (const dish of db.dishes) {
    ...
    dish.submittedAt ||= dish.createdAt || now();
    dish.approvedAt ||= dish.status === "approved" ? dish.submittedAt : null;
    dish.approvedBy ||= "";
    dish.rejectionReason ||= "";
    dish.cookSnapshot ||= dishCookSnapshot(db, db.cooks.find((cook) => cook.id === dish.cookId));
    ...
    dish.featured = Boolean(dish.featured);
  }
  for (const order of db.orders) {
    const amounts = orderAmounts(order.subtotal || order.items?.reduce((sum, item) => sum + Number(item.qty || 1) * Number(item.price || 0), 0) || 0);
    order.subtotal = amounts.subtotal;
    order.appCommissionRate = Number(order.appCommissionRate || amounts.appCommissionRate);
    order.appCommissionAmount = roundMoney(order.appCommissionAmount || amounts.appCommissionAmount);
    order.deliveryFee = roundMoney(order.deliveryFee || amounts.deliveryFee);
    order.totalAmount = roundMoney(order.totalAmount || amounts.totalAmount);
    order.serviceFee = order.appCommissionAmount;
    order.total = order.totalAmount;
    order.statusHistory ||= [];
    order.payment = { ...paymentLedgerForOrder(order), ...(order.payment || {}) };
    order.payment.gross = amounts.totalAmount;
    order.payment.foodAmount = amounts.subtotal;
    order.payment.deliveryFee = amounts.deliveryFee;
    order.payment.appCommissionRate = amounts.appCommissionRate;
    order.payment.appCommissionAmount = amounts.appCommissionAmount;
    order.payment.totalAmount = amounts.totalAmount;
    order.payment.commissionRate = amounts.appCommissionRate;
    order.payment.commission = amounts.appCommissionAmount;
    order.payment.cookPayout = roundMoney(Math.max(0, amounts.subtotal - amounts.appCommissionAmount));
    if (order.payment.status === "held" || order.payment.status === "released") {
      order.payment.paymentStatus ||= order.payment.provider === "cash_on_delivery" || order.payment.method === "cash" ? "cash_on_delivery" : "paid";
      order.payment.payoutStatus ||= order.payment.status === "released" ? "released" : "pending";
      order.payment.status = order.payment.paymentStatus;
    }
    order.payment.paymentStatus ||= order.payment.status || initialPaymentStatus(order.payment.provider || paymentProviderFor(order.paymentMethod || "cash"));
    order.payment.status = order.payment.paymentStatus;
    order.payment.payoutStatus ||= "pending";
    order.paymentStatus = order.payment.paymentStatus;
    order.payoutStatus = order.payment.payoutStatus;
    ...
    order.requestedAt ||= order.createdAt || now();
    order.requestSnapshot ||= {};
  }
  for (const payment of db.payments) {
    const order = db.orders.find((item) => item.id === payment.orderId);
    const amounts = orderAmounts(order?.subtotal || payment.foodAmount || 0);
    payment.method ||= order?.paymentMethod || "cash";
    payment.provider ||= paymentProviderFor(payment.method) === "cash" ? "cash_on_delivery" : paymentProviderFor(payment.method) || "manual";
    if (payment.status === "held" || payment.status === "released") {
      payment.paymentStatus ||= payment.provider === "cash_on_delivery" || payment.method === "cash" ? "cash_on_delivery" : "paid";
      payment.payoutStatus ||= payment.status === "released" ? "released" : "pending";
      payment.status = payment.paymentStatus;
    }
    payment.paymentStatus ||= payment.status || initialPaymentStatus(payment.provider);
    payment.status = payment.paymentStatus;
    payment.payoutStatus ||= "pending";
    payment.gross = amounts.totalAmount;
    payment.foodAmount = amounts.subtotal;
    payment.deliveryFee = amounts.deliveryFee;
    payment.appCommissionRate = amounts.appCommissionRate;
    payment.appCommissionAmount = amounts.appCommissionAmount;
    payment.totalAmount = amounts.totalAmount;
    payment.commissionRate = amounts.appCommissionRate;
    payment.commission = amounts.appCommissionAmount;
    payment.cookPayout = roundMoney(Math.max(0, amounts.subtotal - amounts.appCommissionAmount));
    payment.providerReference ||= payment.externalPaymentId || "";
    payment.metadata ||= {};
    if (order) maybeReleasePayout(order, payment, payment.releasedAt || order.updatedAt || now());
  }
...
const toCook = (row) => ({
  ...
  status: row.status,
  requestStatus: row.request_status || row.status,
...
const fromCook = (cook) => ({
  ...
  country: cook.country || "TR",
  city: cook.city,
  district: cook.district || "",
  full_address: cook.fullAddress || "",
  ...
  status: cook.status,
  request_status: cook.requestStatus || cook.status || "pending",
  ...
  request_date: cook.requestDate || cook.createdAt || now(),
...
const toDish = (row) => ({
  ...
  category: row.category || row.country || (Array.isArray(row.tags) ? row.tags[0] : "") || "",
  tags: row.tags || [],
  ...
  cookSnapshot: row.cook_snapshot || {},
  address: row.address || "",
  location: row.location || null,
...
const fromDish = (dish) => ({
  ...
  category: dish.category || dish.country || dish.tags?.[0] || "",
  tags: dish.tags || [],
  status: dish.status || "approved",
  submitted_at: dish.submittedAt || now(),
  approved_at: dish.approvedAt || null,
  approved_by: dish.approvedBy || null,
  rejection_reason: dish.rejectionReason || "",
  cook_snapshot: dish.cookSnapshot || {},
...
const toOrder = (row) => ({
  ...
  appCommissionRate: Number(row.app_commission_rate || row.commission_rate || commissionRate),
  appCommissionAmount: Number(row.app_commission_amount || row.service_fee || 0),
  ...
  requestSource: row.request_source || "web",
...
const fromOrder = (order) => ({
  ...
  app_commission_rate: order.appCommissionRate || commissionRate,
  app_commission_amount: order.appCommissionAmount || order.serviceFee || 0,
  delivery_fee: order.deliveryFee || 0,
  service_fee: order.serviceFee || order.appCommissionAmount || 0,
  ...
  request_source: order.requestSource || "web",
  requested_at: order.requestedAt || order.createdAt || now(),
  request_snapshot: order.requestSnapshot || {},
  notes: order.notes || "",
...
const toPayment = (row) => ({
  ...
  appCommissionRate: Number(row.app_commission_rate || row.commission_rate || commissionRate),
  appCommissionAmount: Number(row.app_commission_amount || row.commission || 0),
  totalAmount: Number(row.total_amount || row.gross || 0),
  cookPayout: Number(row.cook_payout || 0),
  provider: row.provider || "manual",
  externalPaymentId: row.external_payment_id || "",
  providerReference: row.provider_reference || row.external_payment_id || "",
...
const fromPayment = (payment) => ({
  ...
  payout_status: payment.payoutStatus || "pending",
  gross: payment.gross || 0,
  food_amount: payment.foodAmount || 0,
  delivery_fee: payment.deliveryFee || 0,
  commission_rate: payment.commissionRate || commissionRate,
  commission: payment.commission || 0,
  app_commission_rate: payment.appCommissionRate || payment.commissionRate || commissionRate,
  app_commission_amount: payment.appCommissionAmount || payment.commission || 0,
  total_amount: payment.totalAmount || payment.gross || 0,
  cook_payout: payment.cookPayout || 0,
  provider: payment.provider || "manual",
  external_payment_id: payment.externalPaymentId || "",
  provider_reference: payment.providerReference || payment.externalPaymentId || "",
...
const toAuditLog = (row) => ({
  ...
  oldPayoutStatus: row.old_payout_status || "",
  newPayoutStatus: row.new_payout_status || "",
  provider: row.provider || "",
  providerReference: row.provider_reference || "",
  note: row.note || "",
...
const fromAuditLog = (entry) => ({
  id: entry.id,
  actor_id: entry.actorId || null,
  order_id: entry.orderId || null,
  payment_id: entry.paymentId || null,
  action_type: entry.actionType || "manual_admin_action",
  old_payment_status: entry.oldPaymentStatus || "",
  new_payment_status: entry.newPaymentStatus || "",
  old_payout_status: entry.oldPayoutStatus || "",
  new_payout_status: entry.newPayoutStatus || "",
  provider: entry.provider || "",
...
async function loadSupabaseDb() {
  const [users, cooks, dishes, orders, messages, notifications, sessions, mealPlans, subscriptions, payments, refunds, paymentTimeline, auditLogs, socialActions, authTokens, notificationDevices] = await Promise.all([
    ...
    supabaseRequest("payment_timeline", { query: "?select=*&order=created_at.desc" }).catch(() => []),
    supabaseRequest("audit_logs", { query: "?select=*&order=created_at.desc" }).catch(() => []),
  ...
  return normalizeDb({
    ...
    paymentTimeline: paymentTimeline.map(toTimelineEvent),
...
async function deleteSupabaseDemoData(removed) {
  await deleteSupabaseValues("audit_logs", "id", removed.auditLogs || new Set());
  await deleteSupabaseValues("payment_timeline", "id", removed.paymentTimeline || new Set());
...
async function saveSupabaseDb(db) {
  ...
  await upsert("payment_timeline", (db.paymentTimeline || []).map(fromTimelineEvent)).catch(() => []);
  await upsert("audit_logs", (db.auditLogs || []).map(fromAuditLog)).catch(() => []);
...
function visiblePaymentTimeline(db, user) {
  if (user.role === "owner") return db.paymentTimeline || [];
  const orderIds = new Set(visibleOrders(db, user).map((order) => order.id));
  return (db.paymentTimeline || []).filter((event) => orderIds.has(event.orderId));
}
...
function socialSummary(db, cookId = null) {
  ...
  return {
    followers: new Set(actions.filter((action) => action.type === "follow").map((action) => `${action.userId}:${action.cookId}`)).size,
...
function publicState(db, user = null) {
  ...
  const ownCook = user ? cookForUser(db, user.id) : null;
  const visibleDishes = db.dishes
    ...
    .map((dish) => publicDishForUser(dish, user, ownCook));
  return {
    user: safeUser(user),
    cooks,
    ...
    paymentTimeline: user ? visiblePaymentTimeline(db, user) : [],
...
function publicMarketplaceState(db) {
  ...
  return {
    cooks,
    dishes: db.dishes.filter((dish) => isPublicDish(dish) && cookIds.has(dish.cookId)).map((dish) => publicDishForUser(dish, null, null)),
    social: socialSummary(db),
    time: now()
  };
}
...
function partialPublicState(user) {
  return {
    ...
    notifications: [],
    stats: user?.role === "owner"
      ? { users: 1, cooks: 0, drivers: user.role === "driver" ? 1 : 0, pendingCooks: 0, pendingDishes: 0, orders: 0, revenue: 0, commission: 0, pendingRefunds: 0, activeSubscriptions: 0 }
      : null
  };
}
...
async function fastSupabaseLogin(req, res) {
  ...
  if (!user) {
    ...
    return json(res, 401, { error: "Invalid email or password.", reason: "no_account" });
  }
  if (!isUsablePasswordHash(user.passwordHash)) {
    logLoginFailure("malformed_password_hash", email);
    return json(res, 401, { error: "Invalid email or password.", reason: "malformed_hash" });
  }
  if (!verifyPassword(String(input.password || ""), user.passwordHash)) {
    logLoginFailure("wrong_password", email);
    return json(res, 401, { error: "Invalid email or password.", reason: "wrong_password" });
  }
  const token = id("ses");
...
function findPaymentForProvider(db, provider, input = {}) {
  const orderId = input.orderId || input.order_id || input.metadata?.order_id || input.merchant_oid || "";
  const paymentId = input.paymentId || input.payment_id || input.metadata?.payment_id || input.conversationId || "";
  const externalId = input.externalPaymentId || input.external_payment_id || input.id || input.paymentId || input.token || input.merchant_oid || "";
  return db.payments.find((payment) =>
    (paymentId && payment.id === paymentId) ||
    (orderId && payment.orderId === orderId) ||
    (externalId && (payment.externalPaymentId === externalId || payment.providerReference === externalId)) ||
    (provider === "paytr" && externalId && payment.id.replace(/[^A-Za-z0-9]/g, "") === externalId)
  ) || null;
}
...
async function handleStripeWebhook(req, res, db) {
  if (!stripeWebhookSecret) return json(res, 503, { error: "Stripe webhook secret is not configured." });
  const raw = await rawBody(req);
  if (!verifyStripeSignature(raw, req.headers["stripe-signature"])) {
    addPaymentTimeline(db, { type: "provider_callback_rejected", provider: "stripe", note: "Invalid Stripe signature." });
    addAuditLog(db, { actionType: "provider_callback_rejected", provider: "stripe", note: "Invalid Stripe signature.", requestMeta: { route: "/api/payments/stripe/webhook" } });
    await saveDb(db);
    return json(res, 400, { error: "Invalid Stripe signature." });
  }
  const event = JSON.parse(raw || "{}");
  const object = event.data?.object || {};
  const payment = findPaymentForProvider(db, "stripe", { id: object.id, metadata: object.metadata || {} });
  if (!payment) return json(res, 404, { error: "Payment not found." });
  addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "provider_callback_received", provider: "stripe", providerReference: object.id, note: event.type });
  if (event.type === "payment_intent.succeeded") {
    setPaymentStatus(db, payment, "paid", { externalPaymentId: object.id, providerReference: object.latest_charge || object.id, metadata: { stripeEventId: event.id, stripeStatus: object.status } });
  } else if (event.type === "payment_intent.payment_failed" || event.type === "payment_intent.canceled") {
    setPaymentStatus(db, payment, "failed", { externalPaymentId: object.id, metadata: { stripeEventId: event.id, stripeStatus: object.status } });
  } else {
    payment.metadata = { ...(payment.metadata || {}), lastStripeEvent: event.type, lastStripeEventId: event.id };
  }
  await saveDb(db);
  return json(res, 200, { ok: true });
}

async function handleIyzicoCallback(req, res, db) {
  if (!iyzicoCallbackSecret) return json(res, 503, { error: "iyzico callback secret is not configured." });
  const raw = await rawBody(req);
  const signature = String(req.headers["x-hometaste-signature"] || req.headers["x-iyzi-signature"] || "");
  const expected = crypto.createHmac("sha256", iyzicoCallbackSecret).update(raw).digest("hex");
  if (!signature || !safeCompare(expected, signature)) {
    addPaymentTimeline(db, { type: "provider_callback_rejected", provider: "iyzico", note: "Invalid iyzico callback signature." });
    addAuditLog(db, { actionType: "provider_callback_rejected", provider: "iyzico", note: "Invalid iyzico callback signature.", requestMeta: { route: "/api/payments/iyzico/callback" } });
    await saveDb(db);
    return json(res, 400, { error: "Invalid iyzico callback signature." });
  }
  const input = raw.trim().startsWith("{") ? JSON.parse(raw) : Object.fromEntries(new URLSearchParams(raw).entries());
  const payment = findPaymentForProvider(db, "iyzico", input);
  if (!payment) return json(res, 404, { error: "Payment not found." });
  addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "provider_callback_received", provider: "iyzico", providerReference: input.paymentId || input.token || "", note: String(input.status || input.paymentStatus || "") });
  const success = ["success", "paid"].includes(String(input.status || input.paymentStatus || "").toLowerCase());
  setPaymentStatus(db, payment, success ? "paid" : "failed", { externalPaymentId: input.paymentId || input.token || "", providerReference: input.conversationId || input.paymentId || "", metadata: { iyzico: input } });
  await saveDb(db);
  return json(res, 200, { ok: true });
}

async function handlePaytrCallback(req, res, db) {
  if (!paytrMerchantKey || !paytrMerchantSalt) return json(res, 503, { error: "PayTR callback secrets are not configured." });
  const raw = await rawBody(req);
  const input = Object.fromEntries(new URLSearchParams(raw).entries());
  const expected = crypto.createHmac("sha256", paytrMerchantKey).update(`${input.merchant_oid || ""}${paytrMerchantSalt}${input.status || ""}${input.total_amount || ""}`).digest("base64");
  if (!input.hash || !safeCompare(expected, input.hash)) {
    addPaymentTimeline(db, { type: "provider_callback_rejected", provider: "paytr", providerReference: input.merchant_oid || "", note: "Invalid PayTR callback hash." });
    addAuditLog(db, { actionType: "provider_callback_rejected", provider: "paytr", providerReference: input.merchant_oid || "", note: "Invalid PayTR callback hash.", requestMeta: { route: "/api/payments/paytr/callback" } });
    await saveDb(db);
    return json(res, 400, { error: "Invalid PayTR callback hash." });
  }
  const payment = findPaymentForProvider(db, "paytr", input);
  if (!payment) return json(res, 404, { error: "Payment not found." });
  addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "provider_callback_received", provider: "paytr", providerReference: input.merchant_oid, note: input.status });
  setPaymentStatus(db, payment, input.status === "success" ? "paid" : "failed", { externalPaymentId: input.merchant_oid, providerReference: input.merchant_oid, metadata: { paytr: { status: input.status, totalAmount: input.total_amount } } });
  await saveDb(db);
  return res.end("OK");
}
...
async function api(req, res, pathname) {
  ...
  if (useSupabase && req.method === "POST" && pathname === "/api/auth/login") {
    // Ensure seeded owner/cook/driver accounts exist BEFORE the fast login
    // shortcut queries app_users, so admin/seed login works on first attempt.
    try {
      await ensureSeedUsersReady();
    } catch {
      return json(res, 503, { error: "Login service is not ready. Please try again shortly.", reason: "seed_unavailable" });
    }
    return fastSupabaseLogin(req, res);
  }
  ...
  if (req.method === "POST" && pathname === "/api/payments/stripe/webhook") return handleStripeWebhook(req, res, db);
  if (req.method === "POST" && pathname === "/api/payments/iyzico/callback") return handleIyzicoCallback(req, res, db);
  if (req.method === "POST" && pathname === "/api/payments/paytr/callback") return handlePaytrCallback(req, res, db);
  ...
  // TEMP DEV ONLY — disable before production. Mints a session for the seeded
  // owner without a password so the frontend can skip the login page. Returns 403
  // unless HOMETASTE_BYPASS_LOGIN is enabled on the backend.
  if (req.method === "POST" && pathname === "/api/auth/dev-bypass") {
    if (!bypassLogin) return json(res, 403, { error: "Login bypass is disabled.", reason: "bypass_disabled" });
    if (useSupabase) await ensureSeedUsersReady().catch(() => {});
    // Pick the bypass user by HOMETASTE_BYPASS_USER_EMAIL, else by HOMETASTE_BYPASS_ROLE
    // (default "customer"). For owner/admin roles we only reuse an existing seeded
    // account; for a normal user role we safely create one if missing.
    const wantsPrivileged = ["owner", "admin"].includes(bypassRole);
    let user = null;
    if (bypassUserEmail) user = db.users.find((item) => item.email === bypassUserEmail);
    if (!user && wantsPrivileged) user = db.users.find((item) => item.id === "usr_owner" || item.role === "owner");
    if (!user && !wantsPrivileged) user = db.users.find((item) => item.email === (bypassUserEmail || "user@hometaste.local") && item.role === bypassRole);
    if (!user) {
      if (wantsPrivileged) return json(res, 404, { error: "No owner account is available to bypass into." });
      user = {
        id: id("usr"),
        name: "Demo User",
        email: bypassUserEmail || "user@hometaste.local",
        passwordHash: hashPassword(authToken("bypass_password")),
        role: bypassRole || "customer",
        city: "Istanbul",
        country: "TR",
        phone: "",
        emailVerified: true,
    ...
    db.sessions[token] = { userId: user.id, createdAt: now() };
    await saveDb(db);
    return json(res, 200, { token, state: publicState(db, user), bypass: true });
  }

  const user = requireUser(db, req);
  if (!user) return json(res, 401, { error: "Please sign in first." });
  if (pathname.startsWith("/api/admin/") && user.role !== "owner") {
    return json(res, 403, { error: "Only admin can use this endpoint." });
  }
  ...
  if (req.method === "POST" && pathname === "/api/cooks/apply") {
    ...
    cook = {
      ...
      location: submittedCoordinates(input.location || input.coordinates),
    ...
    user.role = "cook";
    db.cooks.push(cook);
    notifyOwners(db, `${cook.name} applied to become a cook.`, { type: "cook_application", cookId: cook.id, userId: user.id, requestDate: cook.requestDate, status: cook.status });
    await saveDb(db);
  ...
  if (req.method === "POST" && pathname === "/api/dishes") {
    ...
    const input = await body(req);
    const targetCook = input.cookId && user.role === "owner" ? db.cooks.find((item) => item.id === input.cookId) : cook;
    if (!targetCook) return json(res, 404, { error: "Cook not found." });
    const submittedAt = now();
    ...
    const dish = {
      ...
      rejectionReason: "",
      cookSnapshot: dishCookSnapshot(db, targetCook),
      address: String(input.address || input.fullAddress || targetCook.fullAddress || "").trim(),
      location: submittedCoordinates(input.location || input.coordinates) || targetCook.location || null,
      available: true,
      featured: false
    };
    ...
    db.dishes.push(dish);
    if (!isAdminCreated) {
      notifyOwners(db, `${targetCook.name} submitted dish/ad "${dish.name}" for approval.`, {
  ...
  if (req.method === "PATCH" && pathname.startsWith("/api/dishes/")) {
    ...
    if (user.role !== "owner" && ("status" in input || "approvedAt" in input || "approvedBy" in input || "rejectionReason" in input || "featured" in input)) {
      return json(res, 403, { error: "Only admin can change dish approval fields." });
    }
    ...
    if (user.role !== "owner" && "available" in input && targets.some((target) => target.status !== "approved")) {
      return json(res, 400, { error: "Only approved dishes can be turned online or offline." });
    }
    let updatedImage = null;
    if (input.image !== undefined) {
      try {
        updatedImage = validateImageValue(input.image, "Dish photo");
      } catch (err) {
        return json(res, 400, { error: err.message });
      }
    }
  ...
  if (req.method === "POST" && pathname === "/api/orders") {
    ...
    if (!orderCook) return json(res, 404, { error: "Cook not found." });
    ...
    if (!paymentMethods.includes(requestedPaymentMethod)) return json(res, 400, { error: `Unsupported payment method: ${requestedPaymentMethod}` });
    ...
    const cookLocation = coordinateFromText(orderCook.city || "Istanbul");
    const requestSnapshot = orderRequestSnapshot(db, user, firstDish, orderCook, input, createdAt);
    const order = {
      id: id("ord"),
      customerId: user.id,
      customerName,
    ...
    order.payment.status = initialPaymentStatus(order.payment.provider);
    ...
    const payment = {
      ...
      payoutStatus: order.payment.payoutStatus,
      gross: order.payment.gross,
      foodAmount: order.payment.foodAmount,
      deliveryFee: order.payment.deliveryFee,
      commissionRate: order.payment.commissionRate,
      commission: order.payment.commission,
      appCommissionRate: order.payment.appCommissionRate,
      appCommissionAmount: order.payment.appCommissionAmount,
      ...
      metadata: {},
      createdAt: now(),
      capturedAt: order.payment.capturedAt || null,
      releasedAt: null
    };
    let checkout = null;
    if (["stripe", "iyzico", "paytr"].includes(provider)) {
      try {
        ...
        order.payment.providerReference = payment.providerReference;
        order.payment.checkoutUrl = payment.checkoutUrl;
        order.payment.metadata = checkout;
      } catch (err) {
        payment.metadata = { checkoutStatus: "configuration_required", checkoutError: err.message };
        order.payment.metadata = payment.metadata;
      }
    }
    db.orders.unshift(order);
    db.payments.unshift(payment);
    addPaymentTimeline(db, { orderId: order.id, paymentId: payment.id, type: "order_created", actorId: user.id, provider: payment.provider, providerReference: payment.providerReference, note: "Order created." });
    addPaymentTimeline(db, { orderId: order.id, paymentId: payment.id, type: "payment_pending", actorId: user.id, provider: payment.provider, providerReference: payment.providerReference, note: `Payment started as ${payment.paymentStatus}.` });
    const pushNotes = [];
    if (orderCook?.userId) pushNotes.push(notification(db, orderCook.userId, `New order ${order.id} received.`, { orderId: order.id, status: order.status }));
    notifyOwners(db, `Customer order ${order.id} received for ${firstDish.name}.`, {
  ...
  if (req.method === "PATCH" && pathname.startsWith("/api/orders/")) {
    ...
    order.updatedAt = now();
    if (order.status === "accepted") addPaymentTimeline(db, { orderId: order.id, paymentId: db.payments.find((item) => item.orderId === order.id)?.id || "", type: "order_accepted", actorId: user.id, note: "Order accepted." });
    if (order.status === "delivered") {
      ...
      if (payment) maybeReleasePayout(order, payment, order.updatedAt, db);
      else order.payment = { ...(order.payment || paymentLedgerForOrder(order)), payoutStatus: "pending", releasedAt: null };
    }
  ...
  if (req.method === "GET" && pathname === "/api/social/follows") {
    return json(res, 200, {
      follows: db.socialActions.filter((action) => action.type === "follow" && action.userId === user.id).map((action) => action.cookId),
      followerCounts: Object.fromEntries(db.cooks.map((cook) => [cook.id, followerCount(db, cook.id)])),
      state: publicState(db, user)
    });
  }

  if (req.method === "GET" && pathname === "/api/favorites") {
    return json(res, 200, { favorites: userFavorites(db, user.id), state: publicState(db, user) });
  }

  if (req.method === "POST" && (pathname === "/api/social" || pathname === "/api/favorites")) {
    const input = await body(req);
    const type = String(input.type || "").trim();
    if (!["follow", "unfollow", "favorite_cook", "unfavorite_cook", "favorite_dish", "unfavorite_dish", "like", "comment", "photo"].includes(type)) return json(res, 400, { error: "Invalid social action." });
    ...
    if ((type === "follow" || type === "unfollow") && !cookId) return json(res, 400, { error: "Cook is required." });
    if ((type === "favorite_cook" || type === "unfavorite_cook") && !cookId) return json(res, 400, { error: "Cook is required." });
    if ((type === "favorite_dish" || type === "unfavorite_dish") && !dishId) return json(res, 400, { error: "Dish is required." });
    if (type === "follow") {
    ...
    if (type === "unfollow") {
      ...
      if (cook) cook.followers = followerCount(db, cook.id);
      await saveDb(db);
      return json(res, 200, publicState(db, user));
    }
  ...
  if (user.role === "owner" && req.method === "PATCH" && pathname.startsWith("/api/admin/refunds/")) {
    ...
    refund.amount = roundMoney(Number(order?.totalAmount || order?.total || 0) * rate);
    ...
    if (order) {
      ...
      const payment = db.payments.find((item) => item.orderId === order.id);
      if (payment && refund.amount > 0) setPaymentStatus(db, payment, "refunded", { metadata: { refundId: refund.id, refundOutcome: input.outcome } });
    }
  ...
  if (user.role === "owner" && req.method === "POST" && pathname.startsWith("/api/admin/payments/") && pathname.endsWith("/refund")) {
    const paymentId = pathname.split("/").at(-2);
    ...
    if (payment.payoutStatus === "released") {
      addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "manual_admin_action", actorId: user.id, provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: "Refund blocked: payout already released; manual review required." });
      addAuditLog(db, { actorId: user.id, orderId: payment.orderId, paymentId: payment.id, actionType: "refund_blocked_manual_review", oldPaymentStatus, newPaymentStatus: oldPaymentStatus, oldPayoutStatus, newPayoutStatus: oldPayoutStatus, provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: String(input.note || "Payout already released; manual review required."), requestMeta: { route: pathname } });
      await saveDb(db);
      return json(res, 409, { error: "Payout already released. Refund requires manual review and must not silently claw back cook payout." });
    }
    setPaymentStatus(db, payment, "refunded", { actorId: user.id, note: String(input.note || "Admin marked payment refunded.").trim(), requestMeta: { route: pathname } });
    payment.payoutStatus = "refunded";
    const order = db.orders.find((item) => item.id === payment.orderId);
    if (order) syncOrderPayment(order, payment);
    addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "payout_refunded", actorId: user.id, provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: "Pending payout marked refunded." });
    await saveDb(db);
    return json(res, 200, publicState(db, user));
  }

  if (user.role === "owner" && req.method === "PATCH" && pathname.startsWith("/api/admin/payments/")) {
    const payment = db.payments.find((item) => item.id === pathname.split("/").pop());
    if (!payment) return json(res, 404, { error: "Payment not found." });
    const input = await body(req);
    const provider = payment.provider || paymentProviderFor(payment.method);
    if (input.action === "confirm_manual" || input.paymentStatus === "paid") {
      if (isGatewayProvider(provider)) return json(res, 400, { error: "Gateway payments can only be marked paid by a verified provider callback." });
      setPaymentStatus(db, payment, "paid", { actorId: user.id, providerReference: String(input.providerReference || input.reference || "").trim(), note: String(input.note || "Manual payment confirmed.").trim(), metadata: { manualConfirmationNote: String(input.note || "").trim(), confirmedBy: user.id }, requestMeta: { route: pathname } });
      addPaymentTimeline(db, { orderId: payment.orderId, paymentId: payment.id, type: "manual_admin_action", actorId: user.id, provider: payment.provider, providerReference: payment.providerReference || payment.externalPaymentId || "", note: "Manual payment confirmed by admin." });
    } else if (input.paymentStatus === "failed") {
      setPaymentStatus(db, payment, "failed", { actorId: user.id, note: String(input.note || "").trim(), metadata: { adminNote: String(input.note || "").trim(), updatedBy: user.id }, requestMeta: { route: pathname } });
    } else if (input.paymentStatus === "refunded") {
      setPaymentStatus(db, payment, "refunded", { actorId: user.id, note: String(input.note || "").trim(), metadata: { adminNote: String(input.note || "").trim(), updatedBy: user.id }, requestMeta: { route: pathname } });
    } else if (input.payoutStatus === "released") {
      const order = db.orders.find((item) => item.id === payment.orderId);
      if (!maybeReleasePayout(order, payment, now(), db)) return json(res, 400, { error: "Payout cannot be released until payment is confirmed and the order is delivered/completed." });
    } else {
      return json(res, 400, { error: "Unsupported payment admin action." });
    }
    await saveDb(db);
    return json(res, 200, publicState(db, user));
  }
