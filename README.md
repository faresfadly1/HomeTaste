...
SEED_OWNER_PASSWORD=use-a-strong-private-password
SEED_OWNER_NAME=HomeTaste Admin
```
...
2. Add an Authorized redirect URI that matches your backend callback exactly, e.g. `https://hometaste-api-production.up.railway.app/api/auth/oauth/google/callback` (or `http://localhost:4174/api/auth/oauth/google/callback` for local).
